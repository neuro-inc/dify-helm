...
class DifyOutputProcessor(BaseAppOutputsProcessor[DifyAppOutputs]):
    async def _generate_outputs(
        self,
        helm_values: dict[str, t.Any],
        app_instance_id: str,
    ) -> DifyAppOutputs:
        return DifyAppOutputs.model_validate(
            await get_dify_outputs(helm_values, app_instance_id)
        )
